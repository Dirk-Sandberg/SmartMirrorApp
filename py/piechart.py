...
class PieChart(GridLayout):
    def __init__(self, data, legend_enable=True, **kwargs):
...
class LegendTree(GridLayout):
    def __init__(self, data, position, size, **kwargs):
        ...
        for key, value in data.items():
            ...
            self.legend = Legend(pos=(
            self.position[0], self.position[1] - count * self.size[1] * 0.15),
                                 size=self.size,
                                 color=color,
                                 name=key,
                                 value=percentage)
...
class Legend(FloatLayout):
    def __init__(self, pos, size, color, name, value, **kwargs):
        ...
        with self.canvas.before:
            Color(*color)
            self.rect = Rectangle(
                pos=(pos[0] + size[0] * 1.3, pos[1] + size[1] * 0.9),
                size=(size[0] * 0.1, size[1] * 0.1))
            self.label = Label(text=str("%.2f" % value + "% - " + name),
                               pos=(pos[0] + size[0] * 1.3 + size[0] * 0.5,
                                    pos[1] + size[1] * 0.9 - 30),
...
class Pie(FloatLayout):
    def __init__(self, data, **kwargs):
        ...
        for key, value in data.items():
            ...
            color = value[2]
            # add part of Pie
            self.temp.append(PieSlice(angle_start=angle_start,
...
class PieSlice(FloatLayout):
    def __init__(self, angle_start, angle_end, name,
                 **kwargs):
        ...
        with self.canvas.before:
            Color(*color)
            self.slice = Ellipse(pos=self.pos, size=self.size,
    ...
    def move_pie_out(self):
        ang = self.slice.angle_start + (
                    self.slice.angle_end - self.slice.angle_start) / 2
